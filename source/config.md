...
使用 Notepad++，VS Code，Vim 等编辑器打开主题配置文件。对于 Windows 用户，**不建议** 使用自带记事本打开配置文件，因为可能会带来难以预测的编码错误。
...
你可以在此处随意添加信息，所有内容会被追加到 `<head>` 内，例如搜索引擎所有权验证等。
...
显示在封面的标题，如果此处留空将自动 fallback 至站点配置文件的 `title`。
...
与 `sitename` 二选一，展示在封面。如果两项均设置将优先展示 `logo`。
